...
def register_callbacks(app):
    ...
    def update_traders(log_name, current_trader):
        traders = get_marks(log_name)
        options = [{"label": t, "value": t} for t in traders]
    ...
    def update_order_book_plot(log_name, product, display_options, timestamp_range, qty_range, qty_exact, mark_type):
        ...
        fig = build_figure(
            ...
            show_imc_price = "show_imc_price" in display_options,
    ...
    def search_logs(search_term, log_name, product):
        ...
        for row in logs.iter_rows(named=True):
            formatted = format_log(logs, row["timestamp"], row["product"])
            if search_term.lower() in formatted.lower():
