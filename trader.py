...
class BaseTrader:
    def __init__(self, state: TradingState, product: str, logger: Logger):
        ...
        if not self.bids or not self.asks:
            self.logger.log_error("Missing bids or asks — empty order book")

        self.mid: Optional[float] = (self.best_bid + self.best_ask) / 2 if self.best_bid and self.best_ask else None
...
class PureMarketMaker(BaseTrader):
    def get_orders(self):
        # Market make
    ...
    def override_best_bid(self):
        if self.best_bid and self.fair_price and self.best_bid < self.fair_price - 1:
            self.buy(self.best_bid + 1, max_quantity=self.pos_limit)

    def override_best_ask(self):
        if self.best_ask and self.fair_price and self.best_ask > self.fair_price + 1:
            self.sell(self.best_ask - 1, max_quantity=self.pos_limit)
...
TRADERS = {
    ...
    "INTARIAN_PEPPER_ROOT": PureMarketMaker
}
